...
// Helper to pack a set of world keypoints into a mec::UserStateVector, as a
// neighbour's user beacon would arrive (§4.5). ECS-side only; used by the ECS
// demo and tests to inject synthetic LOS-node observations into the
// UserNeighbourTable.
...
inline mec::UserStateVector pack_beacon(
    const std::array<WorldKeypoint, kNumKeypoints>& kps,
    mec::NodeId sender, double now, uint16_t frame_id = 0,
